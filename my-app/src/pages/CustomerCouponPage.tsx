...
import AvailableCouponList, {
  AvailableCoupon,
} from "../Customer/AvailableCouponList";
...
export default function CustomerCouponPage() {
  ...
  const [issuedCouponIds, setIssuedCouponIds] = useState<Set<number>>(
    new Set()
  );
  ...
  const fetchAvailableCoupons = useCallback(async () => {
    ...
    try {
      ...
      const issuedIds = new Set<number>(
        myCouponsResponse.data.map((c: any) => Number(c.couponId))
      );
      ...
      <div className="max-w-2xl mx-auto p-4">
        ...
        {loading ? (
          <div className="flex flex-col items-center py-8 animate-pulse">
            <svg
              className="w-10 h-10 text-blue-400 animate-spin mb-2"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
