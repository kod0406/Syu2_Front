...
export default function CustomerLogin() {
  ...
  const handleResendVerification = async () => {
    try {
      ...
      const errorData = err.response?.data;
      setAlertMessage(errorData?.message || '인증 메일 재발송 중 오류가 발생했습니다.');
    }
  };
      ...
      <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        <div className="space-y-4">
          <div className="flex justify-between text-sm px-4">
            <button onClick={handleSignUp} className="text-blue-600">회원가입</button>
            <button onClick={() => navigate('/forgot-password')} className="text-gray-600">비밀번호 찾기</button>
          </div>
          ...
          <div className="text-center">
            <button onClick={handleGoToResendPage} className="text-sm text-gray-500 underline">이메일 재인증이 필요하신가요?</button>
          </div>
        </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
              <div className="text-center">
                ...
                <div className="space-y-3">
                  <button onClick={handleResendVerification} className="w-full bg-blue-500 text-white py-2 rounded">인증 메일 재발송</button>
                  <button onClick={handleGoToResendPage} className="w-full bg-gray-500 text-white py-2 rounded">인증 페이지로 이동</button>
                  <button onClick={handleCloseAlert} className="w-full border py-2 rounded text-gray-700">닫기</button>
                </div>
              </div>
            </div>
          </div>
