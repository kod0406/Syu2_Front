...
export default function CouponPage() {
  ...
  const [createExpiryType, setCreateExpiryType] = useState<
    "ABSOLUTE" | "RELATIVE"
  >("ABSOLUTE");
  ...
  const handleCreateCoupon = async () => {
    const payload = {
      ...
      discountLimit: createForm.discountLimit
        ? Number(createForm.discountLimit)
        : null,
      minimumOrderAmount: Number(createForm.minimumOrderAmount),
      expiryType: createExpiryType,
      expiryDate:
        createExpiryType === "ABSOLUTE" ? createForm.expiryDate : null,
      expiryDays:
        createExpiryType === "RELATIVE" ? Number(createForm.expiryDays) : null,
    ...
    try {
      ...
      const message =
        error.response?.data?.message || "네트워크 오류 또는 서버 내부 오류";
  ...
  const handleDeleteCoupon = (couponId: number) => {
    setAlertMessage(
      "정말로 이 쿠폰을 삭제하시겠습니까? 발급된 쿠폰은 삭제할 수 없습니다."
    );
