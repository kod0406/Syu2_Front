...
export default function StoreProfileModal({onClose}: StoreProfileModalProps) {
    ...
    // 회원탈퇴 관련 상태
    const [showWithdrawModal, setShowWithdrawModal] = useState(false);
    const [withdrawStep, setWithdrawStep] = useState<'confirm' | 'input'>('confirm');
    const [withdrawInput, setWithdrawInput] = useState('');
    const [isWithdrawing, setIsWithdrawing] = useState(false);
    ...
    // 탭 상태 추가
    const [activeTab, setActiveTab] = useState<'basic' | 'password' | 'address'>('basic');
...
=======
    ...
    const handleWithdrawSubmit = async () => {
        ...
        try {
            const response = await api.delete('/api/stores/withdraw');
            if (response.status === 200) {
                setMessage('회원탈퇴가 완료되었습니다. 잠시 후 메인 페이지로 이동합니다.');
                setMessageType('success');
                setTimeout(() => {
                    window.location.href = '/';
                }, 2000);
            ...
            setMessage(err.response?.data?.message || '회원탈퇴 중 오류가 발생했습니다.');
            setMessageType('error');
        } finally {
            setIsWithdrawing(false);
            setShowWithdrawModal(false);
...
>>>>>>> 41cc38458c1b036ac5df957cb9577c5235bf12c8
                ...
                if (profileData.storeId) {
                    const addressRes = await api.get(`/api/stores/address/${profileData.storeId}`);
...
=======
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (
            ...
            try {
                const response = await api.put('/api/stores/profile', {storeName: formData.storeName});
                if (response.status === 200) {
                    setMessageType('success');
                    setMessage('매장 정보가 저장되었습니다.');
                    setTimeout(() => {
    ...
    const handleSaveAddress = async () => {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                setProfile(prev => prev ? {...prev, address, addressX, addressY, addressPlaceName} : prev);
            }
        } catch (err: any) {
            setMessageType('error');
            setMessage(err.response?.data?.message || '주소 저장 중 오류가 발생했습니다.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    if (isLoadingProfile) {
        return (
            <div
                ...
                <div
                    className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8 animate-slideUp">
                    <div className="flex flex-col items-center justify-center py-8">
                        ...
                        <span className="mt-4 text-gray-700 font-medium">프로필 정보를 불러오는 중...</span>
                    </div>
                </div>
            </div>
        );
            ...
            <div
                ...
                <div
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-red-600 font-medium mb-4">프로필 정보를 불러올 수 없습니다.</p>
                        <button
                            onClick={onClose}
    ...
    return (
        <div
            className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
            <div
                className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-hidden animate-slideUp">
                {/* 헤더 */}
...
=======
                {/* 메시지 */}
                {message && (
                    <div className={`mx-6 mt-6 p-4 rounded-xl border-l-4 animate-slideDown ${
                        messageType === 'success'
                        ...
                        <div className="flex items-center">
                            <svg
                                className={`w-5 h-5 mr-2 ${messageType === 'success' ? 'text-green-600' : 'text-red-600'}`}
                            ...
                            <div className="space-y-6 animate-fadeIn">
                                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                                    <div className="flex items-center">
                                        ...
                                        <span className="text-sm text-blue-800 font-medium">비밀번호 변경은 선택사항입니다</span>
                                    </div>
                                </div>

                                {[
                                    {name: 'currentPassword', label: '현재 비밀번호', key: 'current'},
                                    {name: 'newPassword', label: '새 비밀번호', key: 'new'},
                                    {name: 'confirmPassword', label: '새 비밀번호 확인', key: 'confirm'}
                                ].map((field) => (
                                    <div key={field.name}>
                                        <label className="block text-sm font-semibold text-gray-800 mb-3">
                                            {field.label}
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                disabled={isLoading}
                                                placeholder={field.name === 'currentPassword' ? '비밀번호 변경 시에만 입력' : `${field.label}를 입력하세요`}
                                            />
                                            <button
                                                type="button"
                                                tabIndex={-1}
                                        ...
                                        <div className="border-2 border-gray-200 rounded-xl p-4 bg-gray-50">
                                            <div className="flex items-center justify-between">
                                                <div className="flex-1">
                                                    ...
                                                    {!address && (
                                                        <p className="text-sm text-gray-500 mt-1">매장 주소를 등록해주세요</p>
                                                    )}
                                                </div>
                                                <button
                                                    type="button"
                ...
                <div className="px-6 py-4 bg-red-50 border-t border-red-200">
                    <div className="text-center">
                        <h3 className="text-sm font-semibold text-red-900 mb-2">⚠️ 위험 구역</h3>
                        <p className="text-xs text-red-700 mb-4">
                            회원탈퇴 시 모든 데이터가 영구적으로 삭제되며, 이 작업은 되돌릴 수 없습니다.
                        </p>
                        <button
                            type="button"
            ...
            {showWithdrawModal && (
                <div
                    className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] animate-fadeIn">
                        ...
                        {withdrawStep === 'confirm' ? (
                            <>
                                <div className="text-center mb-6">
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 mb-2">회원탈퇴 확인</h3>
                                    <p className="text-gray-600 text-sm leading-relaxed">
                                        회원탈퇴 시 다음 데이터가 <span className="font-semibold text-red-600">영구적으로 삭제</span>됩니다:
                                    </p>
                                </div>
                                    ...
                                    <ul className="text-sm text-gray-700 space-y-3">
                                        {[
                                            ...
                                            <li key={index} className="flex items-center">
                                                <svg className="w-4 h-4 text-red-500 mr-3" fill="currentColor"
                                                     viewBox="0 0 20 20">
                                ...
                                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
                                    <div className="flex items-start">
                                        <svg className="w-5 h-5 text-amber-500 mr-3 mt-0.5 flex-shrink-0" fill="none"
                                             stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
                                                  d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"/>
                                        </svg>
                                        <div className="text-sm text-amber-800">
                                            <p className="font-medium mb-1">이 작업은 되돌릴 수 없습니다</p>
                                            <p>탈퇴 후 동일한 이메일로 재가입하더라도 기존 데이터는 복구되지 않습니다.</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <>
                                ...
                                <div className="mb-6">
                                    <label className="block text-sm font-medium text-gray-700 mb-2">
                                        매장명: <span className="font-bold text-gray-900">{profile?.storeName}</span>
                                    </label>
                                    <input
                                        type="text"
                                        value={withdrawInput}
                                    ...
                                    />
                                    {withdrawInput && withdrawInput !== profile?.storeName && (
                                        <p className="text-sm text-red-600 mt-2">매장명이 일치하지 않습니다.</p>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col gap-3">
                                    <button
                                        onClick={handleWithdrawSubmit}
                                        disabled={withdrawInput !== profile?.storeName || isWithdrawing}
