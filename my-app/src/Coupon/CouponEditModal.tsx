...
interface Props {
  coupon: Coupon;
  onClose: () => void;
  onSubmit: (couponId: number, form: CouponForm, expiryType: 'ABSOLUTE' | 'RELATIVE') => void;
}
...
export default function CouponEditModal({ coupon, onClose, onSubmit }: Props) {
  ...
  const [form, setForm] = useState<CouponForm>({
    ...
    expiryDate: coupon.expiryDate ? toLocalDateTimeString(coupon.expiryDate) : '',
  ...
  const [expiryType, setExpiryType] = useState<'ABSOLUTE' | 'RELATIVE'>(coupon.expiryType);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded shadow w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <input className="w-full mb-2 border p-2 rounded" placeholder="최대 할인 금액 (선택)"
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button className="px-4 py-2 bg-yellow-500 text-white rounded" onClick={handleSubmit}>수정</button>
        </div>
      </div>
    </div>
  );
