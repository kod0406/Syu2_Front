...
export const useSessionNotification = () => {
    ...
    // WebSocket 연결
    const connectWebSocket = useCallback((topicPath: string) => {
        ...
        try {
            ...
            const connectHeaders = {
                'Authorization': document.cookie.includes('access_token=') ?
                    `Bearer ${document.cookie.split('access_token=')[1]?.split(';')[0]}` : ''
            };
            ...
            stompClient.connect(connectHeaders,
                (frame: Frame) => {
                    ...
                    stompClient.subscribe(topicPath, (message: Message) => {
                        ...
                        try {
                            const notificationData: SessionNotification = JSON.parse(message.body);
    ...
    // 재연결 스케줄링
    const scheduleReconnect = useCallback((topicPath: string) => {
    ...
    const initializeWebSocket = useCallback(async (): Promise<boolean> => {
        try {
            ...
            const serverUrl = getServerUrl();
            const response = await fetch(`${serverUrl}/api/session/connect-websocket`, {
                method: 'POST',
                credentials: 'include',
                headers: {
            ...
            if (!response.ok) {
                console.warn('WebSocket 연결 정보를 가져올 수 없습니다:', response.status);
                // 응답 내용도 로깅
