...
const AvailableCouponList: React.FC<Props> = ({ coupons, onIssue, issuedCouponIds }) => {
  ...
  return (
    <div className="space-y-4">
      {coupons.length === 0 ? (
        <p className="text-gray-500 text-center py-8">발급 가능한 쿠폰이 없습니다.</p>
      ) : (
        coupons.map(coupon => {
          ...
          return (
            <div key={coupon.id} className={`p-4 rounded-lg shadow-md border-l-4 ${canIssue ? 'border-green-500' : 'border-gray-300'} ${!canIssue ? 'bg-gray-50' : 'bg-white'} animate-fade-in`}>
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-bold text-lg">{coupon.couponName}</p>
                  <p className="text-sm text-gray-700 font-semibold">{coupon.storeName}</p>
                </div>
                <button
              ...
              <div className="mt-3 pt-3 border-t border-gray-200 text-sm">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  남은 수량: {coupon.totalQuantity - coupon.issuedQuantity} / {coupon.totalQuantity}
                </p>
              </div>
            </div>
          )
