  ...
  return (
      <div className="space-y-4">
        {coupons.length === 0 ? (
            ...
            coupons.map(coupon => (
                <div key={coupon.id} className={`p-4 rounded-lg shadow-md border-l-4 ${coupon.isUsed ? 'bg-gray-200 border-gray-400 text-gray-500' : 'bg-white border-blue-500'} animate-fade-in`}>
                  <div className="flex justify-between items-center">
                    <div>
                      <p className={`font-bold text-lg ${coupon.isUsed ? 'text-gray-600' : 'text-black'}`}>{coupon.couponName}</p>
                      <p className="text-sm">{coupon.storeName}</p>
                    </div>
                    {coupon.isUsed && (
                        <span className="px-3 py-1 text-sm font-semibold text-white bg-gray-500 rounded-full">사용 완료</span>
                    )}
                  </div>
                  <div className="mt-3 pt-3 border-t border-gray-200">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {coupon.minimumOrderAmount != null ? `${coupon.minimumOrderAmount.toLocaleString()}원 이상 주문 시` : ''}
                    </p>
