...
const ReviewModal: React.FC<ReviewModalProps> = ({ open, menuName, reviews, onClose }) => {
  if (!open) return null;
        ...
        {reviews.length > 0 ? (
          <ul className="space-y-4">
            {reviews.map((review, i) => (
              <li key={i} className="border rounded p-3 md:p-4 text-sm space-y-2">
                <p className="text-yellow-500 font-semibold">⭐ {review.rating.toFixed(1)} / 5</p>
                <p className="text-gray-700 whitespace-pre-wrap break-words">{review.comment}</p>
          ...
          <p className="text-gray-500 text-sm text-center">아직 등록된 리뷰가 없습니다.</p>
        )}
      </div>
    </div>
  );
